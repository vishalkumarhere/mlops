...
class YouTubeAPI:
    ...
    def get_video_stats(self, video_id):
        try:
            ...
            return {
                ...
                'video_id': video_id  # Add video_id to the returned data
            }
        except HttpError as e:
    ...
    def analyze_upload_patterns(self, df):
        ...
        # Calculate best upload times with proper ordering
        best_hours = df.groupby('hour')['views'].agg(['mean', 'count']).reset_index()
        best_hours = best_hours[best_hours['count'] >= 3]  # Filter hours with at least 3 uploads
        best_hours = best_hours.sort_values('mean', ascending=False).head(3)
        
        best_days = df.groupby('day_of_week')['views'].agg(['mean', 'count']).reset_index()
        best_days = best_days[best_days['count'] >= 3]  # Filter days with at least 3 uploads
        best_days['day_of_week'] = pd.Categorical(best_days['day_of_week'], categories=day_order, ordered=True)
        best_days = best_days.sort_values('mean', ascending=False).head(3)
    ...
    def get_trending_videos(self, region_code='US', category_id=None, max_results=50):
        """Get trending videos for a specific region and category using search API"""
        try:
            # Validate parameters
            ...
            # Sort by views and remove duplicates
            ...
            return unique_trending_videos[:max_results]  # Ensure we don't exceed requested max_results
